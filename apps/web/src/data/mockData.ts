/**
 * Phase 45: trimmed to only what's genuinely used as a pre-auth/
 * fallback default — the app logo asset and placeholder identity
 * fields shown only until a real signed-in user's data loads. Fields
 * that used to assert specific, unverifiable claims about a real
 * user's account (a fabricated job title, city, subscription tier,
 * device count, join date, or "encryption key ID") were removed
 * entirely rather than left as fake defaults — see AppContext.tsx's
 * DEFAULT_PROFILE, which now starts a new user with an honestly empty
 * profile instead of someone else's biography.
 */
